...
for lasfile in (filename for filename in os.listdir() if filename[:4]=="attr"):
	...
	outFile = File("Gary-"+lasfile[:-4]+".las", mode = "w", header = inFile.header)
	classification = inFile.classification
	outFile.points = inFile.points

	classification *= 0

	pointCuboid2 =   (inFile.iso > 0.6) & (inFile.lang < 0.1)
	classification[pointCuboid2]=11

	writecondition_conductor = (np.round(inFile.iso,3)>0.5) & (np.round(inFile.iso,3)<0.6) & (np.round(inFile.lang,2)>0.9) 
	...
	plt.savefig('plot'+lasfile[:-4]+'.png')
