...
\begin{figure}[!h]
  ...
  \centering
    \includegraphics[width=0.9\textwidth]{linregwithnoise.png}
    \label{lrwn}
    \newline{Points which form near-straight lines have higher values}
\end{figure}

\begin{figure}[!h]
  \caption{(Just an example of what our signals will look like) Linear regression, $k=50$, $\epsilon = 0.75$}
  \centering
    \includegraphics[width=0.9\textwidth]{linregnonoise.png}
    \label{lrnn}
\end{figure}
\newpage
\newpage
...
\end{displaymath}
of our points. Now, the points $(x_i,y_i,z_i)$ are coplanar if and only if $w_1=w_2=\dots=w_n=0$, or equivalently $\overline w=0$, for some vector $\mathbf{v}\neq \mathbf{0}$.
...
\end{displaymath}
Since $C$ generates a non-negative bilinear form, $\lambda_0,\lambda_1,\lambda_2\geqslant 0$. The points $(x_i,y_i,z_i)$ are coplanar if and only if the matrix $C$ is \emph{not} positive definite, i.e. if $\det C =\lambda_0\lambda_1\lambda_2 =0$. 
...
\begin{figure}[!h]
  ...
  \centering
    ...
    \label{eigent1}
    \newline{Entropy offers a continous version of dimension. 0D and 1D objects (e.g. noise and conductors) have lowest values, rising up to 3D values with highest values (e.g. trees and corners of buidlings)}
\end{figure}
...
\begin{figure}[!h]
  ...
  \centering
    ...
    \label{eigent2}
    \newline{This conductor is of a different type and appears more 2D -- as shown by its higher entropy.}
\end{figure}
\newpage
\begin{figure}[!h]
...
\begin{figure}[!h]
  ...
  \centering
    ...
    \label{enthist}
    \newline{Lower values are given by noise and conductors. The ground and sides of buidlings contribute to the second mode. Vegetation and corners of buildings contribute to the third mode.}
\end{figure}
\newpage 
