...
def corridor(c, conductor_condition, among, R=1, S=2):
	...
	condition = (w_norms<R) & (np.absolute(scale)<S) & among
...
for tile_name in os.listdir():
	if tile_condition(tile_name):
		...
		conductor = corridor(Coords, classn == 1, dims==1, R=0.5, S=2)
		classn[:]=0
		classn[conductor & (dims != 7)]=1
		classn[dims == 7] = 7
