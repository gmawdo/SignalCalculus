...
for file_name in os.listdir():	
	if "Infty" in file_name and file_name[:4]=="attr" and not("jsd" in file_name):
		for index, item in enumerate(A):
			...
			df = pd.DataFrame({'A': inv, 'C': (classn==index).astype(int)})
			...
			intensity = 1000*X[inv,0]/cnt[inv]
			...
			#unq, ind, inv, cnt = np.unique(np.round(Coords/2,0), return_index=True, return_inverse=True, return_counts=True, axis=1)
			#df['A'] = inv
			...
			out = File(str(index)+"ptbyptjsd"+file_name, mode = "w", header = inFile.header)
			...
			out.close()
			os.chdir("..")
			print("Index",index,file_name, "done")
